...
def get_capital_indices(words):
    """Aquires indices of words with capital letters in the first part of the word for recapitalization
    >>> get_capital_indices(['I','like','to','eat','Bananas', 'and', 'mcDonalds'])
    [0,4]
    """
    #for loop? List comprehension?


def make_initial_letters_lowercase(words):
    """Sets first letter in each word in words to be lowercase.
    >>> make_initial_letters_lowercase(['I','like','to','eat','Bananas', 'MackleMore', 'mcDonalds'])
    ['i', 'like', 'to', 'eat', 'bananas', 'mackleMore', 'mcDonalds']
    """
    # for loop? List comprehension?

def translate_to_pig_latin(words):
    """Translates each word in words to pig latin
...
def re_case_words(words,indices):
    """Makes the first letter uppercase in each word in words list that was previously uppercased"""
    >>> re_cased_pig_latin_words(['i','like','to','eat','bananas', 'mackleMore', 'mcDonalds'],[0,4,5])
    (['I', 'like', 'to', 'eat', 'Bananas', 'MackleMore', 'mcDonalds'])
# for loop? List comprehension?
...
def output(words):
    """Prints words.
    >>> output(['I', 'like', 'to', 'eat', 'Bananas', 'MackleMore', 'mcDonalds'])
    I like to eat Bananas MackleMore mcDonalds
    """
    print(" ".join(words))
...
capital_indices_in_english_words = get capital indices(english_words)
lowercase_words = make_initial_letters_lowercase(english_words)
pig_latin_words = translate_to_pig_latin(lowercase_words)
    cut_and_save_punctuation
    alter_words
    paste_ay
re_cased_pig_latin_words = re_case_words(pig_latin_words,capital_indices_in_english_words )
