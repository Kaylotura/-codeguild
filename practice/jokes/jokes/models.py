...
class Joke:
    """
    Value type that represents a submitted joke. It has a setup, and a punchline, and each submitted joke has a unique
    id generated by the joke_counter.
    """
    joke_counter = 0
    def __init__(self, setup, punchline):
        self.setup = setup
        self.punchline = punchline
        self.id = Joke.joke_counter
        Joke.joke_counter += 1

    def __eq__(self, other):
        return (
            self.setup == other.setup and
            self.punchline == other.punchline and
            self.id == other.id
            )

    def __repr__(self):
        return 'Joke({!r}, {!r}, {!r})'.format(self.setup, self.punchline, self.id)
